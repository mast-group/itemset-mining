...
public class AlgoAgrawal {

	// Database as itemset tree (used for calculating supports)
	ItemsetTree tree;
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns, String output,
			ItemsetTree tree, double minconf) throws IOException {
		// save the parameters
		...
		// start the algorithm
		return runAlgorithm(patterns, output, tree);
	}
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns, String output,
			ItemsetTree tree, double minconf, double minlift)
			throws IOException {
		// save the parameters
		this.minconf = minconf;
	...
	private AssocRules runAlgorithm(Itemsets patterns, String output,
			ItemsetTree tree) throws IOException {
		...
		this.tree = tree;
		this.databaseSize = tree.getNoTransactions();
		...
		for (int k = 2; k < patterns.getLevels().size(); k++) {
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				for (int item : lk.getItems()) {
					...
					int[] itemset_Lk_minus_hm_P_1 = ArraysAlgos
	...
	/**
	 * Calculate the support of an itemset by looking at the patterns of the
	 * same size otherwise get the support from the itemset tree.
	 * 
	 * @param itemset
	 *            the itemset.
	...
	private int calculateSupport(int[] itemset) {
		...
		// Look up support in itemset tree if not in list of itemsets
		// FIXME slow, requires conversion to Itemset class
		return tree.getSupportOfItemset(new itemsetmining.itemset.Itemset(
				itemset));
	}
	...
	public void printStats() {
		System.out
				.println("=============  ASSOCIATION RULE GENERATION v0.96f- STATS =============");
	 ...
	 */
	protected void saveRule(int[] itemset1, int supportItemset1,
			int[] itemset2, int supportItemset2, int absoluteSupport,
